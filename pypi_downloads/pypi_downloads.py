...
class pypi_downloads:
    ...
    def __init__(self, username, sep=';', verbose=3):
        ...
        for repo in repos:
            try:
                ...
                if os.path.isfile(pathname):
                    if self.verbose>=3: print('[pypi_downloads] >[%s] is also found on disk. Merge -> Update -> Write' %(repo))
                    # Read repo from disk
                    df_disk = read_repo_from_disk(pathname, self.sep)
    ...
    def stats(self, repo=None):
        ...
        for repo, pathname in zip(repos, pathnames):
            df = read_repo_from_disk(pathname, self.sep)
...
def read_repo_from_disk(pathname, sep):
